...
def train(epoch, model, optimizer):
    ...
    for i, (data_source, label_source) in enumerate(source_train_generator.__getitem__()):
        #print(data_source.shape, label_source.shape)
        ...
        data_target = target_train_generator.__getitem__()
...
def validate(model):
    with torch.no_grad():
        ...
        for i, (data, target) in enumerate(target_valid_generator.__getitem__()):
...
if __name__ == '__main__':
    ...
    #generate_data_patches(source_train_x_data, source_train_y_data, options['source_h5_path'], options['source_train_csv_path'], options, dataset_name='miccai')
    source_train_files, source_train_files_ref, source_train_patches = load_data_patches(options['source_h5_path'], options['source_train_csv_path'], phase='all', fold=fold)
    source_train_generator = DatasetGenerator(data=source_train_files, options=options, patches=source_train_patches)

    #generate_data_patches(target_valid_x_data, target_valid_y_data, options['target_h5_path'], options['target_train_csv_path'], options, dataset_name='ISBI')
    target_valid_files, target_valid_files_ref, target_valid_patches = load_data_patches(options['target_h5_path'], options['target_train_csv_path'], phase='valid', fold=fold)
    target_valid_generator = DatasetGenerator(data=target_valid_files, options=options, patches=target_valid_patches)

    generate_data_patches(target_train_x_data, None, options['target_h5_path'], options['target_train_csv_path'], options, dataset_name='ISBI')
    target_train_files, target_train_files_ref, target_train_patches = load_data_patches(options['target_h5_path'], options['target_train_csv_path'], phase='train', fold=fold)
    target_train_generator = DatasetGenerator(data=target_train_files, options=options, patches=target_train_patches, phase='test')
    ...
    len_source_train_dataset = source_train_generator.__len__() * options['batch_size'] # len(source_train_loader.dataset)
    len_source_train_loader = source_train_generator.__len__()

    len_target_train_dataset = target_train_generator.__len__() * options['batch_size']
    len_target_train_loader = target_train_generator.__len__()

    len_target_valid_dataset = target_valid_generator.__len__() * options['batch_size'] #len(target_valid_loader.dataset)
    len_target_valid_loader = target_valid_generator.__len__() #len(target_valid_loader)
